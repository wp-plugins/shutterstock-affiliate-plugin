...
function is_shutter_login_correct(){
            ...
            if (($output !== false) && ($output !== ''))  return true;
            else                    return false;
}
            ...
            if (   intval($search_term)>0   )
                $params = array(
                    ...
                    'search_group' => $search_withing,
                     'searchterm'   =>  $shutterstock_nothing_found_keyword_for_contributor
                );
             else
                $params = array(
...
//                        echo '<input onclick="javascript:location.href=\''.$purchase_url.'\';" type="button" value="'.__('Buy','shutterstock').'" />';
             ...
             //   echo '.shutterstock_sub_container{ width:'.(3*$my_width+200).'px;}';
                ...
                $shutterstock_nothing_found_keyword_std = explode(',',$shutterstock_nothing_found_keyword_std);
                $shutterstock_nothing_found_keyword_std = trim($shutterstock_nothing_found_keyword_std[ rand(0 , (count($shutterstock_nothing_found_keyword_std)-1) ) ]);

                if ($shutterstock_nothing_found == 'error') {
                    echo '<h2>' .__('Nothing found', 'shutterstock')  . '</h2>';
                } else {

                    if($shutterstock_nothing_found == 'popular')
                        shutter_show_search_results( $shutterstock_nothing_found_keyword_std,   1 , 'popular' );
                    if($shutterstock_nothing_found == 'random')
                        shutter_show_search_results( $shutterstock_nothing_found_keyword_std,   1 , 'random' );
                    if($shutterstock_nothing_found == 'newest')
                        shutter_show_search_results( $shutterstock_nothing_found_keyword_std,   1 , 'newest' );
                    if($shutterstock_nothing_found == 'keyword')
                        shutter_show_search_results( $shutterstock_nothing_found_keyword,   1 , $_POST['sort_metod'] );
                    if($shutterstock_nothing_found == 'contributor')
                        shutter_show_search_results( $shutterstock_nothing_found_contributor,   1 , $_POST['sort_metod']  );

                    if($shutterstock_nothing_found == 'nothing') return;
                    return;
                }
            }
